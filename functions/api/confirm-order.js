export async function onRequestPost({ request, env }) {
  try {
    const { sessionId, cart, deliveryFee, processingFee = 0 } = await request.json()
    ...
    // ── Pull order data from Stripe metadata ───────────────────────────────
    ...
    const method      = meta.method || 'pickup'
    const methodLabel = method === 'pickup' ? '🏪 Pickup' : '🚗 Delivery'

    const subtotal = cart.reduce((s, i) => s + i.price * i.qty, 0)
    const tax      = subtotal * 0.0825
    const total    = subtotal + tax + (deliveryFee || 0) + (processingFee || 0)
      ...
      <table width="580" cellpadding="0" cellspacing="0" border="0" style="max-width:580px;width:100%;">
        ...
        <tr><td bgcolor="#1a1a1a" style="background-color:#1a1a1a;border-radius:12px;padding:32px;">
          ...
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
            ...
            ${(processingFee || 0) > 0 ? `<tr>
              <td style="color:#888;font-size:0.9rem;padding:4px 0;">Processing Fee (3%)</td>
              <td></td>
              <td align="right" style="color:#888;font-size:0.9rem;padding:4px 0;">$${(processingFee).toFixed(2)}</td>
            </tr>` : ''}
...
<body style="font-family:Arial,sans-serif;background:#f9f9f9;padding:32px;">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e0e0e0;">
    <h2 style="margin:0 0 4px;color:#1a1a1a;">🔔 New Order — PAID ✅</h2>
    ...
    ${(deliveryFee || 0) > 0 ? `<p style="margin:4px 0;color:#555;">Delivery Fee: <strong>$${(deliveryFee).toFixed(2)}</strong></p>` : ''}
    ${(processingFee || 0) > 0 ? `<p style="margin:4px 0;color:#555;">Processing Fee (3%): <strong>$${(processingFee).toFixed(2)}</strong></p>` : ''}
